defmodule DocteurWeb.ClientAuthTest do
  use DocteurWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Docteur.Profile
  alias DocteurWeb.ClientAuth
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, DocteurWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_client/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      DocteurWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount: ensure_authenticated" do
    test "authenticates current_client based on a valid client_token", %{conn: conn, client: client} do
      client_token = Profile.generate_client_session_token(client)
      session = conn |> put_session(:client_token, client_token) |> get_session()
