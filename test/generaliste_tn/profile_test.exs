defmodule GeneralisteTN.ProfileTest do
  use GeneralisteTN.DataCase

  alias GeneralisteTN.Profile

  import GeneralisteTN.ProfileFixtures
  alias GeneralisteTN.Profile.{Client, ClientToken}
  ...
  describe "update_client_email/2" do
    ...
    test "does not update email if client email changed", %{client: client, token: token} do
      assert Profile.update_client_email(%{client | email: "current@example.com"}, token) ==
               :error
  ...
  describe "reset_client_password/2" do
    ...
    test "updates the password", %{client: client} do
      {:ok, updated_client} =
        Profile.reset_client_password(client, %{password: "new valid password"})
